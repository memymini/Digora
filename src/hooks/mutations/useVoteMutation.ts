...
export const submitVote = async ({ voteId, optionId }: { voteId: number, optionId: number }) => {
  return http.post<null>(`/api/votes/${voteId}/vote`, { optionId } as VoteRequest);
};
  ...
  return useApiMutation(
    ({ optionId }: { optionId: number }) => submitVote({ voteId, optionId }),
    {
      onSuccess: () => {
        ...
        queryClient.invalidateQueries({ queryKey: VOTE_QUERY_KEYS.detail(voteId) });
